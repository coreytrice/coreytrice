	...
	function init(){
		if(!empty($this->post_types)){
			...
			foreach($this->options['pstructs'] as $k => $struct){
				// PLUGIN HACK - To suppress php warning, check if $this->post_types[$k]->rewrite is_array()  - ZL
				if(is_array($this->post_types[$k]->rewrite)) {
					...
					// PLUGIN HACK - To suppress notice in phpunit test, check array key exists ['ep_mask'] - ZL
					if (array_key_exists('ep_mask', $this->post_types[$k]->rewrite)) {
						add_permastruct( $k, $struct, $with_front, $this->post_types[$k]->rewrite['ep_mask'] );
					}
				}
			}
		}
